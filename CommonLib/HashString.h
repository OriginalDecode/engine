...
namespace cl
{
	...
	public:
		...
		int32 find(const char* literal) const;
		int32 find(const CHashString* string) const;
		bool contains(const char* literal) const;

		int32 length() const;
		CHashString substr(const int32 start_index, const int32 len) const;
		const char* c_str() const;
		uint64 GetHash() const;

	private:

		int32 Horspool(const char* pattern) const;
	...
	template< int32 _SIZE /*= 30*/>
	uint64 CHashString<_SIZE>::GetHash() const
	{
		return m_HashValue;
	}

	template< int32 _SIZE /*= 30*/>
	CHashString<_SIZE> CHashString<_SIZE>::substr(const int32 start_index, const int32 len) const
	{
	...
	template< int32 _SIZE /*= 30*/>
	int32 CHashString<_SIZE>::find(const CHashString* string) const
	{
	...
	template< int32 _SIZE /*= 30*/>
	int32 CHashString<_SIZE>::find(const char* literal) const
	{
		return Horspool(literal);
	}

	template< int32 _SIZE /*= 30*/>
	char CHashString<_SIZE>::operator[](const int32 index) const
	{
	...
	{
		int32 len = strlen(aLiteralString);
	...
	template< int32 _SIZE /*= 30*/>
	int32 CHashString<_SIZE>::length() const
	{
		return m_Length;
	}

	template< int32 _SIZE /*= 30*/>
	int32 CHashString<_SIZE>::Horspool(const char* pattern) const
	{
		uint64 table[UCHAR_MAX + 1];
		int32 pattern_size = strlen(pattern);
	
		for (int32 i = 0; i <= UCHAR_MAX; ++i)
		{
