...
void Texture::Initiate(const TextureDesc& desc, bool create_from_texture, const cl::HashString& debug_name)
{
	...
	if (desc.m_RenderTargetFormat != graphics::NO_FORMAT)
	{
		...
		tex_desc.m_Binding = desc.m_ResourceTypeBinding; 
		if (create_from_texture)
			m_RenderTargetView = device.CreateShaderResource(tex_desc, m_Texture, debug_name + "_RTV");
		else
			m_RenderTargetView = device.CreateShaderResource(m_Texture, debug_name + "_RTV");
	}
	...
	if (desc.m_DepthTextureFormat != graphics::NO_FORMAT)
	{
		...
		tex_desc.m_Binding = desc.m_ResourceTypeBinding;
		if (create_from_texture)
			m_DepthStencilView = device.CreateShaderResource(tex_desc, m_Texture, debug_name + "_DSV");
		else
			m_DepthStencilView = device.CreateShaderResource(m_Texture, debug_name + "_DSV");
	}
}
