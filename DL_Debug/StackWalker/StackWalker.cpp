...
BaseStackWalker::BaseStackWalker(DWORD dwProcessId, HANDLE hProcess)
{
	...
	m_hProcess = hProcess;
	m_sw = StackWalkerInternal(this, m_hProcess);
...
BOOL BaseStackWalker::ShowCallstack(int depth, HANDLE hThread, const CONTEXT *context, PReadProcessMemoryRoutine readMemoryFunction, LPVOID pUserData)
{
	CONTEXT c;;
...
#error "Platform not supported!"
	...
	for (frameNum = 0; frameNum < depth; ++frameNum)
	{
		...
		{
			OnCallstackEntry(et, csEntry, frameNum);
		}

		if (s.AddrReturn.Offset == 0)
		{
			OnCallstackEntry(lastEntry, csEntry, frameNum);
...
void BaseStackWalker::OnCallstackEntry(CallstackEntryType eType, CallstackEntry &entry, int depth)
{
...
void BaseStackWalker::OnOutput(char* buffer, int depth)
{
	OutputDebugStringA(buffer);
	if (depth < 4)
		m_Callstack[depth] = buffer;
}

StackWalkerInternal::StackWalkerInternal(BaseStackWalker *parent, HANDLE hProcess)
{
	m_parent = parent;
	m_hDbhHelp = NULL;
	pSC = NULL;
...
BOOL StackWalkerInternal::Init(LPCSTR szSymPath)
{
	...
	if (pSGSP != NULL)
	{
		if (pSGSP(m_hProcess, buf, BaseStackWalker::STACKWALK_MAX_NAMELEN) == FALSE)
...
DWORD StackWalkerInternal::LoadModule(HANDLE hProcess, LPCSTR img, LPCSTR mod, DWORD64 baseAddr, DWORD size)
{
	...
	if ((m_parent != NULL) && (szImg != NULL))
	{
		...
		if ((m_parent->m_options & BaseStackWalker::RetrieveFileVersion) != 0)
		{
...
BOOL StackWalkerInternal::GetModuleInfo(HANDLE hProcess, DWORD64 baseAddr, IMAGEHLP_MODULE64_V2 *pModuleInfo)
{
	if (pSGMI == NULL)
	{
		SetLastError(ERROR_DLL_INIT_FAILED);
