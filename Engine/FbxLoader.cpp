...
#define L_PI 3.14159265358979323846264338327
...
bool FillData(ModelData* someData, FbxNode* aNode, AnimationData* aAnimation)
{
	...
	{
		if (someData->mSubMeshes.Capacity() == 0)
		{
			someData->mSubMeshes.Init(1);
		}
		ModelData::SubMesh *temp;
		temp = new ModelData::SubMesh;
	...
	if (someData->mAllByControlPoint)
	{
		...
		for (int lIndex = 0; lIndex < lPolygonVertexCount; ++lIndex)
		{
			...
			fixMatrix = CU::Math::CreateReflectionMatrixAboutAxis44(CU::Vector3f(1, 0, 0));
...
CU::Matrix44f CreateMatrix(FbxAMatrix& aOriention)
{
	...
	returnMatrix.myMatrix[0] = aOriention.GetRow(0).mData[0];
	returnMatrix.myMatrix[1] = aOriention.GetRow(0).mData[1];
	returnMatrix.myMatrix[2] = aOriention.GetRow(0).mData[2];
	returnMatrix.myMatrix[3] = aOriention.GetRow(0).mData[3];

	returnMatrix.myMatrix[4] = aOriention.GetRow(1).mData[0];
	returnMatrix.myMatrix[5] = aOriention.GetRow(1).mData[1];
	returnMatrix.myMatrix[6] = aOriention.GetRow(1).mData[2];
	returnMatrix.myMatrix[7] = aOriention.GetRow(1).mData[3];

	returnMatrix.myMatrix[8] = aOriention.GetRow(2).mData[0];
	returnMatrix.myMatrix[9] = aOriention.GetRow(2).mData[1];
	returnMatrix.myMatrix[10] = aOriention.GetRow(2).mData[2];
	returnMatrix.myMatrix[11] = aOriention.GetRow(2).mData[3];

	returnMatrix.myMatrix[12] = aOriention.GetRow(3).mData[0];
	returnMatrix.myMatrix[13] = aOriention.GetRow(3).mData[1];
	returnMatrix.myMatrix[14] = aOriention.GetRow(3).mData[2];
	returnMatrix.myMatrix[15] = aOriention.GetRow(3).mData[3];
...
CU::Matrix44f GetAnimationMatrix(FbxNode* aNode, FbxAnimLayer* aCurrentAnimLayer, float aTime, CU::Matrix44f& offsetMatrix)
{
	...
	return returnMatrix;
	CU::Vector3f startTranslation = offsetMatrix.GetPosition();
	CU::Vector3f translation = offsetMatrix.GetPosition();
	FbxTime time;
	...
	if (rotationX)
	{
		rotation[0] = -rotationX->Evaluate(time)*(L_PI / 180.0f);
	}
	if (rotationY)
	{
		rotation[1] = rotationY->Evaluate(time)*(L_PI / 180.0f);
	}
	if (rotationZ)
	{
		rotation[2] = -rotationZ->Evaluate(time)*(L_PI / 180.0f);
	}
	CU::Matrix44f rotationMatrix;
	rotationMatrix.SetPosition(translation);
	...
	returnMatrix *= CU::Matrix44f::CreateRotateAroundX(rotation[0]);
	returnMatrix *= CU::Matrix44f::CreateRotateAroundY(rotation[1]);
	returnMatrix *= CU::Matrix44f::CreateRotateAroundZ(rotation[2]);
...
void LoadNodeRecursive(FbxModelData* aModel, AnimationData& aAnimation, FbxNode* aNode, FbxAMatrix& aParentOrientation, FbxPose* aPose, FbxAnimLayer* aCurrentAnimLayer, int parentBone)
{
	...
	CU::Matrix44f fixMatrix;
	fixMatrix = CreateReflectionMatrixAboutAxis44(CU::Vector3f(1, 0, 0));
	...
	if (lNodeAttribute)
	{
		...
		else if (lNodeAttribute->GetAttributeType() == FbxNodeAttribute::eLight)
		{
			...
			newLight->myColor = CU::Vector3f(color.mData[0], color.mData[1], color.mData[2]);
...
FbxModelData* FBXLoader::loadModel(const char* aFile)
{
	...
	DL_PRINT("[FBXLoader](loadModel) : FBXLoader Loading Animations...");
