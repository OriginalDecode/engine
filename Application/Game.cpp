...
void Game::Initiate(const std::string& level)
{
	...
	CameraHandle::GetInstance()->Initiate(&m_Orientation);// &m_Camera->GetPixelOrientation());// /* this should be the player, or a child matrix to the player (relative position with an offset that can rotate around the player object) */);
