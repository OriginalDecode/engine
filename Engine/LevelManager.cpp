...
namespace Cache
{
	...
	Level* LevelManager::LoadLevel(const std::string& level_filepath)
	{
		//m_Engine->GetEntityManager()->CleanUp();
		//m_Engine->GetEntityManager()->Initiate();

		JSONReader read_level(level_filepath);
	...
	void LevelManager::CreateEntitiy(const std::string& entity_filepath, JSONReader& level_reader, JSONElement::ConstMemberIterator it)
	{
		JSONReader entityReader(entity_filepath);
		Entity e = m_EntityManager->CreateEntity();
	}

	void LevelManager::CreateTerrain(const std::string& terrain_path)
	{
