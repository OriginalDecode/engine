...
namespace graphics
{
	...
	void DirectX11::CreateDepthBuffer()
	{
		...
		hr = myDevice->CreateTexture2D(&depthDesc, NULL, &m_DepthBuffer);
		...
		hr = myDevice->CreateDepthStencilView(m_DepthBuffer, &stencilDesc, &m_DepthView);
	...
	void DirectX11::CreateBackBuffer()
	{
		...
		hr = myDevice->CreateRenderTargetView(backbuffer, NULL, &m_RenderTarget);
	...
	void DirectX11::SetSamplerState(const eSamplerStates& samplerState)
	{
		...
		m_Context->PSSetSamplers(0, 1, &mySamplerStates[u16(samplerState)]);
	}

	void DirectX11::SetVertexShader(CompiledShader* vertex_shader)
	{
		m_Context->VSSetShader(vertex_shader ? static_cast<IVertexShader*>(vertex_shader->m_Shader) : nullptr, nullptr, 0);
	}

	void DirectX11::SetPixelShader(CompiledShader* pixel_shader)
	{
		m_Context->PSSetShader(pixel_shader ? static_cast<IPixelShader*>(pixel_shader->m_Shader) : nullptr, nullptr, 0);
	}

	void DirectX11::SetGeometryShader(CompiledShader* geomtery_shader)
	{
		m_Context->GSSetShader(geomtery_shader ? static_cast<IGeometryShader*>(geomtery_shader->m_Shader) : nullptr, nullptr, 0);
	}

	void DirectX11::SetHullShader(CompiledShader* hull_shader)
	{
		m_Context->HSSetShader(hull_shader ? static_cast<IHullShader*>(hull_shader->m_Shader) : nullptr, nullptr, 0);
	}

	void DirectX11::SetDomainShader(CompiledShader* domain_shader)
	{
		m_Context->DSSetShader(domain_shader ? static_cast<IDomainShader*>(domain_shader->m_Shader) : nullptr, nullptr, 0);
	}

	void DirectX11::SetComputeShader(CompiledShader* compute_shader)
	{
		m_Context->CSSetShader(compute_shader ? static_cast<IComputeShader*>(compute_shader->m_Shader) : nullptr, nullptr, 0);
	}
...
// 			myEngineFlags[u16(eEngineFlags::FULLSCREEN)] = FALSE;
	...
	void DirectX11::OnResize()
	{
		...
		hr = factory->CreateSwapChain(myDevice, &scDesc, &m_Swapchain);
