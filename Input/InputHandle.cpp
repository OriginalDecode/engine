...
InputCommand* InputHandle::HandleInput()
{
	if (m_Controller->IsConnected())
	{
		...
		if (m_Controller->LeftThumbstickY() > 0.2f) return m_LThumbYP;
		if (m_Controller->LeftThumbstickY() < -0.2f) return m_LThumbYN;
		if (m_Controller->LeftThumbstickX() > 0.2f) return m_LThumbXP;
		if (m_Controller->LeftThumbstickX() < -0.2f) return m_LThumbXN;
	}
	if (m_Input->Update())
	{
