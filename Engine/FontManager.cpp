...
namespace Snowblind
{
	...
	void CFontManager::LoadFont(const char* aFontPath, short aFontHeight)
	{
		...
		for (int i = 0; i < 128; i++) // I want to loop the first 256 characters
		{
			...
			data.pSysMem = gData;
			data.SysMemPitch = width * aFontHeight;
			...
			std::stringstream ss;
			ss << "Glyph_" << i << ".png";
			HRESULT hr = D3DX11SaveTextureToFile(CEngine::GetInstance()->GetAPI()->GetContext(), texture, D3DX11_IMAGE_FILE_FORMAT::D3DX11_IFF_PNG, ss.str().c_str());
